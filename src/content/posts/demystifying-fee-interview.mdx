...
- **Web fundamentals:** JavaScript, browser, CSS and html questions. Basically, anything you could find in the <Link external={true} href="https://developer.mozilla.org/en-US/">MDN docs</Link> is up for grabs. This includes everything from explaining the event loop to giving a quick demo on flex vs grid. This category is immense but at a high level, reviewing the aforementioned docs, reading up on <Link href="https://www.w3.org/WAI/standards-guidelines/wcag/" external={true}>WGAC</Link> standards, and practicing JS questions on a site like <Link href="https://bigfrontend.dev/" external="true">bigfrontend.dev</Link> are all great ways to prepare for this portion of the interview.
...
- **System design:** Architecting complex, performant web applications; usually these questions are only asked for more senior positions. Unlike traditionally SDE system design interviews, these interviews typically focus on designing an application from the client side perspective. For example, breaking down the process of building a complex UI like the Facebook news feed or a Pinterest board. The <Link href="https://www.youtube.com/@FrontEndEngineer" external={true}>YouTube channel "Front End Engineer"</Link> has a great series of examples and is criminally under-viewed.

- **Data structures & algorithms:** The bread and butter of the quintessential coding interview. These questions are what people typically think of when they think of coding interviews (inverting binary trees, finding the shortest paths, locating the total number of single moms in your area, etc.). Essentially, applying specific data structures & algorithms to the type of questions you'd find on <Link href="https://leetcode.com/" external={true}>leetcode.com</Link>. Like I mentioned earlier, this is typically a small portion of the interview compared to SDE interviews (e.g. only 1-2 rounds of 6 as opposed to 4-5), but it is still present. Practice makes perfect.
